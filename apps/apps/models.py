...
class SubCategory(models.Model):
    ...
    img = models.ImageField(upload_to=lambda instance, filename: '/'.join(
        ['subcategory_meta', str(uuid.uuid1()) + '_' + filename]))
...
class App(models.Model):
    app_id = models.CharField(default=lambda: str(
        uuid.uuid1()), max_length=36, editable=False)
...
class CarouselApp(models.Model):
    ...
    description = models.TextField(max_length=1000)
    img = models.ImageField(upload_to=lambda instance, filename: '/'.join(
        ['carousel_app_meta', str(uuid.uuid1()) + '_' + filename]))
